...
export default function NumericReview({ q, submitAnswer, onComplete, trainingMode = false }) {
  ...
  return (
    <section style={{ display: "grid", gap: "18px", margin: "0 auto", maxWidth: "680px", padding: "24px" }}>
      ...
      {result && <div style={{ display: "grid", gap: "12px" }}><div style={{ color: result.correct ? "#9af0b2" : "#ff9e9e", fontWeight: 800 }}>{result.correct ? "Bonne réponse" : `Réponse attendue : ${result.expected}`}</div>{result.correct && <div style={{ color: "#f2d09b" }}>Solution : {result.expected}</div>}{result.format_error && <div style={{ color: "#f2b56b" }}>Saisis uniquement un nombre (virgule, point ou notation scientifique).</div>}{result.correct && !trainingMode ? <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>{qualities.map(([quality, label]) => <button key={quality} type="button" onClick={() => commit(quality)} disabled={busy} style={{ background: "#242424", border: "1px solid #555", borderRadius: "9px", color: "#fff", cursor: "pointer", padding: "10px 14px" }}>{label}</button>)}</div> : !result.correct && !trainingMode ? <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}><button type="button" onClick={() => commit(0)} disabled={busy} style={{ background: "#442020", border: "1px solid #7f1d1d", borderRadius: "9px", color: "#fff", cursor: "pointer", padding: "10px 14px" }}>Again</button><button type="button" onClick={() => commit(1)} disabled={busy} style={{ background: "#463418", border: "1px solid #92400e", borderRadius: "9px", color: "#fff", cursor: "pointer", padding: "10px 14px" }}>Close</button></div> : <button type="button" onClick={() => commit(0)} disabled={busy} style={{ background: "#242424", border: "1px solid #555", borderRadius: "9px", color: "#fff", cursor: "pointer", padding: "10px 14px", width: "fit-content" }}>{trainingMode && result.correct ? "Suivant" : "Continuer"}</button>}</div>}
