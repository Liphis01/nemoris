...
export default function GridReview({ group, submitAnswer, onComplete, trainingMode = false }) {
  ...
  return <section style={{ display: "grid", gap: "18px", margin: "0 auto", maxWidth: "980px", padding: "24px" }}>
    ...
    {result && <div style={{ display: "grid", gap: "10px" }}><strong style={{ color: allCorrect ? "#9af0b2" : "#ff9e9e" }}>{allCorrect ? "Bonnes réponses" : "Correction affichée"}</strong>{anyHit && !trainingMode ? <div style={{ display: "flex", gap: "8px" }}>{qualities.map(([quality, label]) => <button key={quality} type="button" onClick={() => commit(quality)} disabled={busy} style={{ background: "#242424", border: "1px solid #555", borderRadius: "9px", color: "#fff", padding: "10px 14px" }}>{label}</button>)}</div> : allMiss && !trainingMode ? <div style={{ display: "flex", gap: "8px" }}><button type="button" onClick={() => commit(0)} disabled={busy} style={{ background: "#442020", border: "1px solid #7f1d1d", borderRadius: "9px", color: "#fff", padding: "10px 14px" }}>Again</button><button type="button" onClick={() => commit(1)} disabled={busy} style={{ background: "#463418", border: "1px solid #92400e", borderRadius: "9px", color: "#fff", padding: "10px 14px" }}>Close</button></div> : <button type="button" onClick={() => commit(0)} disabled={busy} style={{ background: "#242424", border: "1px solid #555", borderRadius: "9px", color: "#fff", padding: "10px 14px", width: "fit-content" }}>{trainingMode && allCorrect ? "Suivant" : "Continuer"}</button>}</div>}
